...
PLUGIN_PACKAGE_NAME="pulumi-resource-openfaas-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
go build \
   -ldflags "-X github.com/pulumi/pulumi-openfaas/pkg/version.Version=${VERSION}" \
   -o "${WORK_PATH}/pulumi-resource-openfaas${BIN_SUFFIX}" \
   "${ROOT}/cmd/pulumi-resource-openfaas"
...
CREDS_JSON=$(aws sts assume-role \
                 ...
                 --role-session-name "upload-plugin-pulumi-resource-openfaas" \
